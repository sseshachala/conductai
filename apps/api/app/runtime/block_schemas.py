...
BLOCK_SCHEMAS: dict[str, BlockDef] = {
    ...
    "logic": {
        ...
        "icon": "git-branch",
        "fields": [
            _f("config.condition", "Condition", "select", required=True,
               options=[
                   {"value": "exit_code == 0",        "label": "Previous step succeeded (exit code 0)"},
                   {"value": "exit_code != 0",        "label": "Previous step failed (exit code ≠ 0)"},
                   {"value": "status == 'success'",   "label": "Previous step status = success"},
                   {"value": "status == 'failed'",    "label": "Previous step status = failed"},
                   {"value": "status == 'skipped'",   "label": "Previous step status = skipped"},
                   {"value": "output contains 'yes'", "label": "Output contains 'yes'"},
                   {"value": "output contains 'no'",  "label": "Output contains 'no'"},
               ]),
            _f("config.label", "Label", "text"),
        ],
    },
    ...
    "memory": {
        ...
        "fields": [
            ...
            _f("config.value", "What to store", "text",
               hint="What should be saved — can reference earlier block outputs e.g. {{summarise.output}}"),
            _f("config.label", "Label", "text"),
        ],
    },
