...
class Executor:
    ...
    def _tool_get_approval(self, id: str):
        """One approval request with full tool_input payload."""
        try:
            rid = uuid.UUID(id)
        except ValueError:
        ...
        ws_uuid = uuid.UUID(self.workspace_id)
        r = (
            self.db.query(GuardApprovalRequest)
            .filter(GuardApprovalRequest.id == rid, GuardApprovalRequest.workspace_id == ws_uuid)
            .first()
        )
        if not r:
    ...
    def _tool_list_installed_packs(self):
        """Installed skill packs for this workspace's org."""
        org_ws = _org_ws_subquery(self.db, self.workspace_id)
        rows = (
            self.db.query(WorkspaceSkillPack)
            .filter(WorkspaceSkillPack.workspace_id.in_(org_ws))
            .order_by(WorkspaceSkillPack.installed_at.desc())
            .all()
        )
        return [
            {
    ...
    def _tool_browse_marketplace(self, query: str | None = None, limit: int = 30):
        """Available skill packs in the catalog. Latest version per slug."""
    ...
    def _tool_get_pack_details(self, slug: str):
        """One skill pack's latest version with full rule list."""
        r = (
            self.db.query(SkillPack)
    ...
    def _tool_list_integrations(self):
        """List all integrations (Slack, GitHub, Okta, Vercel, ...) for this workspace."""
        ws_uuid = uuid.UUID(self.workspace_id)
    ...
    def _tool_get_integration_status(self, service: str):
        """One integration by service (github/slack/okta/...) — status + handle + last used."""
        ws_uuid = uuid.UUID(self.workspace_id)
        r = (
            self.db.query(Integration)
            .filter(Integration.workspace_id == ws_uuid, Integration.service == service.lower())
            .first()
        )
        if not r:
    ...
    def _tool_get_member(self, clerk_user_id: str):
        """One member's workspace record."""
        ws_uuid = uuid.UUID(self.workspace_id)
        r = (
            self.db.query(WorkspaceUser)
            .filter(WorkspaceUser.workspace_id == ws_uuid, WorkspaceUser.clerk_user_id == clerk_user_id)
            .first()
        )
        if not r:
    ...
    def _tool_get_audit_events(
        ...
        """Platform audit events (invites, role changes, run triggers, credential
        edits, etc). Separate from Guard audit — this is org-wide platform activity."""
        ws_uuid = uuid.UUID(self.workspace_id)
    ...
    def _tool_search_audit_log(self, q: str, limit: int = 25):
        """Substring search across audit action, actor_email, resource_type, resource_id."""
        ws_uuid = uuid.UUID(self.workspace_id)
        like = f"%{q.lower()}%"
        rows = (
            self.db.query(AuditLog)
            .filter(
                AuditLog.workspace_id == ws_uuid,
                sa_or(
                    sa_func.lower(AuditLog.action).like(like),
    ...
    def _tool_list_projects(self, limit: int = 50):
        ...
        rows = (
            ...
            .filter(Project.workspace_id == ws_uuid)
    ...
    def _tool_list_alerts(
        ...
        """Watchdog alerts (stale worker, credential expiry, silent playbook, etc)."""
        ws_uuid = uuid.UUID(self.workspace_id)
    ...
    def _tool_get_alert(self, id: str):
        """One watchdog alert with full payload."""
        try:
            aid = uuid.UUID(id)
        except ValueError:
            return {"error": "id must be a UUID"}
        ws_uuid = uuid.UUID(self.workspace_id)
        r = (
            self.db.query(WatchdogEvent)
            .filter(WatchdogEvent.id == aid, WatchdogEvent.workspace_id == ws_uuid)
            .first()
        )
        if not r:
    ...
    def _tool_list_run_events(self, run_id: str, kind: str | None = None, limit: int = 100):
        """Events emitted during a workflow run — block_started/completed/failed/
        approval_requested/etc. Use when the user wants to see 'what happened
        during run X'."""
        try:
            rid = uuid.UUID(run_id)
        except ValueError:
            return {"error": "run_id must be a UUID"}
        # Scope check — only return events for runs in this workspace's org.
        from app.models.run import Run
        org_ws = _org_ws_subquery(self.db, self.workspace_id)
        run = (
            self.db.query(Run)
            .filter(Run.id == rid, Run.workspace_id.in_(org_ws))
            .first()
        )
        if not run:
