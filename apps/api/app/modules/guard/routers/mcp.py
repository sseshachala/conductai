...
def _match_policy(
    ...
    # #1770: proxy rules can carry match_prompt / match_provider / match_model
    # (LLM-egress matchers). When the caller supplies these in tool_input
    # (guard_check_prompt does), respect them so proxy rules match the same
    # way they do through the LLM proxy PEP itself.
...
def _project_rule(r: dict) -> dict:
    ...
    out = {
        ...
        # #1770: proxy-native matchers preserved so guard_check_prompt can
        # honour them from the MCP transport. Legacy MCP callers pass no
