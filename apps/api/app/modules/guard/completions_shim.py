"""Profile-native ``POST /gateway/v1/completions`` shim (#2144, PR 1).
...
profiles land in PR 2 with a canonical → Anthropic Messages converter and a
response normalizer back to canonical shape.
...
- ``profile`` MUST match ``cond-<8chars>-<alias>`` exactly. Anything else
  ...
  on the v1 code path.
- ``stream`` MUST be strictly ``false`` (or omitted). Anything truthy —
  ``1``, ``"false"``, ``"no"`` — is rejected. Streaming lands in a
  follow-up PR from the #2144 chain.
...
class _CanonicalRequest(BaseModel):
    ...
    # Literal[False] means only the JSON literal ``false`` is accepted.
    # ``stream: 1`` / ``"false"`` / ``"no"`` all trip validation here,
    # not later inside the executor's ``bool(...)`` coercion.
...
async def gateway_completions_impl(
    ...
    PR 1: OpenAI-target profiles only. Body is validated by the strict
    canonical schema, then ``profile`` is rewritten to ``model`` so the
    v2 executor's existing cond_code parser can pick the profile. No
    other conversion is needed because the canonical envelope IS the
