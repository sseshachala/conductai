"""
POST /sdd/generate — spec generation (free public tool, authenticated users unrestricted)

Unauthenticated callers are rate-limited via Redis: 5 requests per IP per day.
Authenticated users (Bearer token or API key) bypass the rate limit entirely.
Redis rate limit survives deploys and is shared across all workers.
"""
import datetime
import structlog
...
def _check_rate_limit(ip: str) -> None:
    """Redis-backed sliding-window rate limit — shared across all workers."""
    try:
        import redis as _redis_mod
        r = _redis_mod.from_url(settings.redis_url, decode_responses=True, socket_connect_timeout=1)
        key = f"sdd:rl:{ip}"
        count = r.incr(key)
        if count == 1:
        ...
        # Redis unavailable — fail open with a warning (don't block the user)
        log.warning("sdd.rate_limit_redis_unavailable", error=str(exc), ip=ip)
...
def get_questions(
    ...
    request: Request,
    user_id: Annotated[str | None, Depends(_get_optional_user_id)] = None,
) -> QuestionsResponse:
