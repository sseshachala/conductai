...
export default function GatewayProfileSettings({ workspaceId, isAdmin }: { workspaceId: string; isAdmin: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (next) {
        ...
        const refParts = String(next.credential_ref || "").match(/^vault:\/\/([0-9a-f-]{36})\//i)
        const legacyVaultId = String(next.credential_ref || "").startsWith("vault://")
          ? next.environment_id || envRows.find(env => env.name === "Default")?.id
          : ""
        setVaultId(refParts?.[1] || legacyVaultId || "")
  ...
  function body() {
    return {
      ...
      credential_ref: vaultId ? `vault://${vaultId}/${provider}` : null,
  ...
  return (
    <div style={{ maxWidth: 700, display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <label style={{ fontSize: 12 }}>Vault
        <select value={vaultId} disabled={!isAdmin} onChange={e => setVaultId(e.target.value)} style={inputStyle}>
