...
const PASSTHROUGH_INTEGRATION_OPERATIONS: Record<string, Operation[]> = {
  ...
  // portkey (PR 4) / azure_openai (PR 6) / custom (PR 7) stay
  // uncertified in this mirror until they ship. Absent =
  // deriveAccepts contributes nothing for them.
}
  ...
  return (
    <div className="card" style={{ padding: 12, display: "grid", gridTemplateColumns: "auto 1fr 1fr 1fr 1fr auto", gap: 10, alignItems: "end" }}>
      ...
      {target.transport === "http_passthrough" ? (
        <FieldLabel
          label="Integration"
          hint="External gateway routing traffic on our behalf. OpenRouter + Helicone (OpenAI/Anthropic) are certified today; Azure OpenAI + Custom stay uncertified until each ships its per-integration auth shape. Helicone vault entries must hold two keys: HELICONE_API_KEY + the vendor key (OPENAI_API_KEY or ANTHROPIC_API_KEY)."
        >
