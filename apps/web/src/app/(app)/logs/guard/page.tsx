      ...
      {activeView === "sessions" && (
        <div className="card" style={{ overflow: "hidden" }}>
          <div style={{
            ...
            {["Agent identity", "Tool", "Started", "Events", "Violations", "Cost", "Saved", "Machine / IP", "OS"].map((h, i) => (
        ...
        reportsError ? (
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1.2fr 0.6fr 0.6fr 0.7fr 0.6fr 0.9fr 0.5fr", gap: 12, padding: "10px 18px", borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>
              {["Agent identity", "Archetype", "Sessions", "Commits", "Autonomy", "Lines/hr", "Date", "Report"].map(h => (
      ...
      {activeView === "events" && (loading ? (
        ...
        (() => {
          ...
          const tableHeader = (
            ...
            >
              {["Time", "Agent identity", "Tool", "Call", "Input", "Decision", "Rule", "Blast Radius"].map((h, i) => (
                <div
                  key={i}
                  className="eyebrow"
                  style={{ fontSize: 9.5 }}
