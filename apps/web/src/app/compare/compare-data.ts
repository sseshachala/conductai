export const TOOLS = [
  ...
  },
  {
    ...
    description: "Runtm provides the execution infrastructure for coding agents. It answers the question 'where does the agent actually run?' — not 'what should the agent do?' You spin up an isolated sandbox, point your agent at it (Claude Code, Cursor, Codex, Gemini CLI), the agent codes inside the box, and Runtm deploys the result to a live HTTPS endpoint. It includes real-time observability, session logs, cost tracking, approval gates, and team collaboration. Where Conduct is the orchestration layer that decides what an agent should do and when to do it, Runtm is the runtime layer where the code execution happens. The two are complementary — Conduct's brain blocks can route to Runtm as a sandbox backend instead of (or alongside) Modal.",
    strengths: [
      "Isolated sandboxes — agents get full permissions with zero risk to your machine or cloud",
      "Works with any coding agent — Claude Code, Cursor, Codex, Gemini CLI, and more",
      "Live HTTPS deploy URLs out of the box — test and share instantly",
      "Real-time session logs and observability built-in",
