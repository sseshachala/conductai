# Sentinel — AI Tool Fleet Management for Engineering Teams
**v0.3 · May 2026**
...
Sentinel is that system for Claude Code, Codex, Cursor, and Gemini CLI.
...
sentinel join <team-invite-code>
...
If a developer uses a blocked tool, Sentinel logs it and notifies the team lead.
```
...
When a Conduct agent run starts, Sentinel governance and optimization apply automatically — no extra config in the playbook.

```
Conduct run starts
  → Sentinel middleware loads team policy (same policy as developer sessions)
...
The manager sees Conduct agent runs in the same Sentinel dashboard as developer sessions. One view for all AI activity — human-driven or automated.
...
```
1. Sign up at sentinel.conductai.ai
...
```bash
sentinel join abc-xyz-123      # invite code from team lead
# → installs hooks for Claude Code, Codex, Cursor
# → downloads current team policy
# → done
```

From that point: policy updates, optimizer changes, budget changes — all push down automatically. Developer never touches Sentinel again unless they want to check their own spend.
...
Manager changes a rule in dashboard
        ↓ (within 60 seconds)
Sentinel hook on every developer machine polls for config update
        ↓
...
- Sentinel middleware in Conduct agent runs
- Same policy applied to automated runs
- Conduct runs appear in Sentinel dashboard alongside developer sessions
...
Your IT team manages your laptop fleet. Nobody expects each developer to configure their own security policy. Sentinel does the same for Claude Code and Codex — the team lead is in control, developers just work.
