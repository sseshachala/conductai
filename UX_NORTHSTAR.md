...
| `/settings/agents` *(new)*                              | Settings     |      |          |       |      | **build new** — turn budgets, model tier defaults, memory defaults, fallback model order, per-role agent permissions (decision #27) |
| `/settings/guard`                                       | Settings     |      | x        |       |      | `/guard/settings` moves here so Guard spine stays 4 tabs    |
